...
# Use /tmp for cloud deployment (writable), fallback to local for development
DB_PATH = "/tmp/expenses.db" if os.environ.get("FASTMCP_CLOUD_URL") else os.path.join(os.path.dirname(__file__), "expenses.db")
CATEGORIES_PATH = os.path.join(os.path.dirname(__file__), "categories.json")
...
async def list_all_expenses():
    '''List ALL expense entries ever recorded, no date range needed.'''
    try:
        async with aiosqlite.connect(DB_PATH) as c:
            cur = await c.execute(
                "SELECT id, date, amount, category, subcategory, note FROM expenses ORDER BY date DESC, id DESC"
            )
            cols = [d[0] for d in cur.description]
            rows = [dict(zip(cols, r)) for r in await cur.fetchall()]
            if not rows:
                return {"status": "empty", "message": "No expenses recorded yet."}
            return rows
    except Exception as e:
...
# ← __main__ block REMOVED — FastMCP Cloud handles transport automatically